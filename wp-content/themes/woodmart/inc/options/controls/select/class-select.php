	...
	public function render_control() {
		...
		<select class="xts-select<?php echo esc_attr( $classes ); ?>" name="<?php echo esc_attr( $name ); ?>" <?php echo $attributes; // phpcs:ignore ?>>
			...
			<?php foreach ( $options as $key => $option ) : ?>
				...
				if ( is_array( $value ) && in_array( $key, $value, false ) ) { // phpcs:ignore
					$selected = true;
				} elseif ( ! is_array( $value ) && strval( $value ) === strval( $key ) ) {
					$selected = true;
				}
				
				?>
				<option value="<?php echo esc_attr( $key ); ?>" <?php selected( true, $selected ); ?>><?php echo esc_html( $option['name'] ); ?></option>
			<?php endforeach ?>
		</select>
		<?php
	}
